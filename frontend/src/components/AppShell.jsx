    ...
    <div className="min-h-screen lg:grid lg:grid-cols-[290px_minmax(0,1fr)]">
      <aside className="border-b border-white/10 bg-ink-900/95 p-6 lg:min-h-screen lg:border-b-0 lg:border-r">
        ...
        <div className="mt-8 rounded-3xl border border-white/10 bg-white/5 p-4">
          ...
          <label className="mt-4 block text-xs uppercase tracking-[0.2em] text-sand-100/45">{t('locale_label')}</label>
          <select className="field mt-2" onChange={(event) => setLocale(event.target.value)} value={locale}>
